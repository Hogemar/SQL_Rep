...
ALTER TABLE Талон DISABLE TRIGGER After_CheckPaymentAmount
ALTER TABLE Талон ENABLE TRIGGER After_CheckPaymentAmount
...
AS
    ...
    IF @TotalInserted > @TotalValid
		PRINT('Внимание: ' + CONVERT(varchar(20), @TotalInserted - @TotalValid) + ' записей не были добавлены, так как сумма оплаты превышает 1000 рублей.')
GO
...
AS
    ...
    IF @TotalValid > 0
        PRINT ('Удалено записей: ' + CONVERT(varchar(20), @TotalValid) + '. Общее количество записей: ' + CONVERT(varchar(20), @TotalDeleted))
GO

DELETE FROM Пациент
	WHERE [Страховой полис] IN (1,2,10)
